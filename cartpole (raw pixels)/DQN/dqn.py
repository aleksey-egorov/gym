...
class DQN_Conv:
    def __init__(self, env, conv_config, fc_config):
        self.env = env
        self.Q_loss_list = []

        self.net = Network(conv_config, fc_config, device)
        self.net_target = Network(conv_config, fc_config, device)
    ...
    def select_action(self, state, epsilon):
        if np.random.random() < epsilon:
            ...
            #print ("STATE_V={}".format(state_v.shape))
