...
from A3C_Conv.models import A3C_Model
from A3C_Conv.utils import unpack_batch
...
class A3C_Conv():
    ...
    def __init__(self, state_dim, action_dim, entropy_beta, gamma, bellman_steps, batch_size, clip_grad):
        ...
        self.model = A3C_Model(state_dim, action_dim).to(device)
    ...
    def update(self, batch):
        ...
        states_v, actions_t, q_vals_v = \
            unpack_batch(batch, self.model, last_val_gamma=self.gamma ** self.bellman_steps, device=device)
        batch.clear()
        ...
        return adv_v, value_v, q_vals_v, entropy_loss_v, loss_policy_v, loss_value_v, loss_v
