...
from Dueling_DDQN_PER_CNNLSTM.models import DuelingNetwork

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


class Dueling_DDQN_PER_CNNLSTM:
    def __init__(self, env, input_shape, n_actions, batch_size):
        self.env = env
        self.batch_size = batch_size
        self.Q_loss_list = []

        self.net = DuelingNetwork(input_shape, n_actions, batch_size)
        self.net_target = DuelingNetwork(input_shape, n_actions, batch_size)
