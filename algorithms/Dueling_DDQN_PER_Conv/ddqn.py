...
from Dueling_DDQN_PER_Conv.models import DuelingNetwork

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


class Dueling_DDQN_PER_Conv:
    def __init__(self, env, input_shape, n_actions, config):
        self.env = env
        self.Q_loss_list = []

        self.net = DuelingNetwork(input_shape, n_actions, config, device)
        self.net_target = DuelingNetwork(input_shape, n_actions, config, device)
